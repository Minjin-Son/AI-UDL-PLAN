...
import { exportPlanAsWord, exportPlanAsHwp, exportPlanAsPpt } from '../services/exportService';
...
const PlanDisplay: React.FC<{
  ...
  const handleHwpExport = () => {
    exportPlanAsHwp(plan, view);
    setIsExportMenuOpen(false);
  }

  const handlePptExport = () => {
    exportPlanAsPpt(plan, view);
    setIsExportMenuOpen(false);
  }
                  ...
                  {isExportMenuOpen && (
                    <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                      <div className="py-1">
                        ...
                        <button onClick={handleHwpExport} className="text-slate-700 block w-full text-left px-4 py-2 text-sm hover:bg-slate-100 flex items-center gap-2">
                        ...
                        <button onClick={handlePptExport} className="text-slate-700 block w-full text-left px-4 py-2 text-sm hover:bg-slate-100 flex items-center gap-2">
        ...
        {view === 'udl' && (
          <>
            ...
            {editablePlan.multimedia_resources && (
              <MultimediaLinks resources={editablePlan.multimedia_resources} />
            )}
          </>
        )}
        {view === 'table' && (
...
const DisplayPanel: React.FC<DisplayPanelProps> = (props) => {
  ...
  if (props.isLoading) {
    ...
    content = <PlanDisplay
      ...
      onRevisionRequest={props.onRevisionRequest}
      isRevising={props.isRevising}
    />;
  } else {
