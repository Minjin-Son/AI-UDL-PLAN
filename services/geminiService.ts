        ...
        try {
            ...
            const prompt = `
              ...
              **수정 지침:**
              ...
              3.  (중요!) UDL 3원칙의 명칭은 반드시 **'참여의 원리', '표상의 원리', '행동 및 표현의 원리'**를 사용해야 합니다.
              4.  결과물은 반드시 기존과 동일한 JSON 스키마 형식(${JSON.stringify(responseSchema)})으로 반환해야 합니다. 모든 내용은 한국어로 작성해주세요.
              5.  결과물에는 **설명이나 \`\`\`json \`\`\` 같은 불필요한 텍스트 없이 순수한 JSON 객체만** 포함해야 합니다.
            `;
